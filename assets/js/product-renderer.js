(function () {
  ...
  if (typeof rawCsvData !== 'undefined') {
    allProducts = parseCSV(rawCsvData);
  }
  ...
  function renderFilteredProducts() {
    ...
    const filtered = allProducts.filter(p => {
      if (!p['BRAND NAME']) return false;

      const rawBrandName = p['BRAND NAME'];
      const category = p['CATEGORY'] || 'General';
      const brandName = `${rawBrandName} ${capitalize(category)}`;
      const packSize = p['PACK SIZE'] || 'N/A';
      const division = p['Division'] || 'ultima';

      const filterDiv = division.toLowerCase();
      let filterCat = 'other';
      const catLower = category.toLowerCase();
      
      const rawBrandLower = rawBrandName.toLowerCase();
      const isOralSolidOrInj = catLower.includes('tablet') || catLower.includes('capsule') || catLower.includes('softgel') || catLower.includes('injection');
      
      const isOrthoOtc = (
        catLower.includes('ortho') || catLower.includes('knee') || catLower.includes('guard') || 
        catLower.includes('pain spray') || catLower.includes('pain oil') || catLower.includes('pain gel') || catLower.includes('hot gel') ||
        rawBrandLower.includes('ventoran 2x') || rawBrandLower.includes('ventoran pain') || rawBrandLower.includes('dolanta hot gel') || rawBrandLower.includes('dolanta pain gel')
      ) && !isOralSolidOrInj;

      if (isOrthoOtc) {
        filterCat = 'ortho-support';
      } else if (catLower.includes('tablet')) {
        filterCat = 'tablets';
      } else if (catLower.includes('softgel') || catLower.includes('softgl')) {
        filterCat = 'soft-gels';
      } else if (catLower.includes('capsule')) {
        filterCat = 'capsules';
      } else if (catLower.includes('injection') || catLower.includes('inj')) {
        filterCat = 'injections';
      } else if (catLower.includes('syrup') || catLower.includes('liquid') || catLower.includes('suspension') || catLower.includes('emulsion') || catLower.includes('solution') || catLower.includes('soluution') || catLower.includes('oral solution')) {
        filterCat = 'liquids';
      } else if (catLower.includes('sachet') || catLower.includes('powder') || catLower.includes('granules') || catLower.includes('ors')) {
        filterCat = 'oral-powder';
      } else if (catLower.includes('drop') || catLower.includes('spray') || catLower.includes('condom') || catLower.includes('lozenge')) {
        filterCat = 'drops-sprays';
      } else if (catLower.includes('oint') || catLower.includes('cream') || catLower.includes('lotion') || catLower.includes('soap') || catLower.includes('paste') || catLower.includes('wash') || catLower.includes('gel') || catLower.includes('shampoo') || catLower.includes('facewash')) {
        filterCat = 'topicals';
      } else if (catLower.includes('respule') || catLower.includes('mdi')) {
        filterCat = 'respules-mdis';
      } else {
        filterCat = 'topicals';
      }

      const brandMatch = activeBrand === 'all' || filterDiv === activeBrand;
      const categoryMatch = activeCategories.length === 0 || activeCategories.includes(filterCat);
      ...
      const searchIndex = [
        ...
        .join(" ")
        .toLowerCase();
      const searchMatch = !searchTerm || searchIndex.includes(searchTerm);
    ...
    if (visibleProducts.length === 0) {
      html = `
        <div class="col-12 text-center py-5 product-empty-state">
      ...
      visibleProducts.forEach(p => {
        ...
        html += `
          <div
            class="col-lg-4 col-md-4 col-sm-6 col-12 product-card-col"
            data-brand="${filterDiv}"
            data-brand-label="${capitalize(division)}"
            data-category="${filterCat}"
            data-category-label="${capitalize(category)}"
            data-product-name="${brandName}"
  ...
  function initProductFilters() {
    const searchInput = document.querySelector(".control-search-input");
    ...
    const getActiveCategories = () => {
      activeCategories = categoryCheckboxes
    ...
    categoryCheckboxes.forEach(cb => {
      cb.addEventListener('change', () => {
        const checkedCount = categoryCheckboxes.filter(item => item.checked).length;
        if (checkedCount === 0) {
          cb.checked = true;
        }
        getActiveCategories();
